...
export const TourInfoForm: FC<IProps> = ({ calcId, formProps, servicesOptions, brandOptions }) => {
  ...
  const fetchCities = async (query: string, setSuggestions: React.Dispatch<React.SetStateAction<string[]>>) => {
    ...
    try {
      const response = await fetch(
        `https://wft-geo-db.p.rapidapi.com/v1/geo/cities?namePrefix=${query}&limit=10&languageCode=ru`,
        {
          method: 'GET',
          headers: {
            'X-RapidAPI-Key': process.env.REACT_APP_GEODB_API_KEY as string,
  ...
  return (
    ...
    >
      <Loading isSpin={isLoading}>
        <form className={styles.form}>
          {/* ... остальная разметка формы без изменений ... */}
        </form>
      </Loading>
    </Accordion>
  );
