...
export const TourInfoForm: FC<IProps> = ({ calcId, formProps, servicesOptions, brandOptions }) => {
  ...
  const fetchEuropeanCountries = async () => {
    const url = 'https://gas-price.p.rapidapi.com/europeanCountries'; // Правильный URL
    const options = {
      method: 'GET',
      headers: {
        'X-Rapidapi-Key': '37dd5627b6msh043bf027fda8958p102480jsn08fd44a67f3a', // Ваш ключ API
        'X-Rapidapi-Host': 'gas-price.p.rapidapi.com' // Хост API
   ,
    };
    try {
      ...
      // Здесь можно обработать данные, например, вывести в UI
    } catch (error) {
      console.error('Error fetching European countries:', error);
    }
  };
  fetchEuropeanCountries(); // Вызов функции
