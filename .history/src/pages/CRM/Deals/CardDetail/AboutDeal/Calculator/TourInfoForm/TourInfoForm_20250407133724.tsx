...
export const TourInfoForm: FC<IProps> = ({ calcId, formProps, servicesOptions, brandOptions }) => {
  ...
  fetchCity();
  useEffect(() => {
    if (!departureCity) return; // Если поле пустое, ничего не делаем
    const delayDebounceFn = setTimeout(() => {
      fetchCities(departureCity, setDepartureSuggestions);
    }, 300); // Задержка в 300ms

    return () => clearTimeout(delayDebounceFn); // Очищаем таймер при изменении
  }, [departureCity]);

  useEffect(() => {
    if (!arrivalCity) return; // Если поле пустое, ничего не делаем
    const delayDebounceFn = setTimeout(() => {
      fetchCities(arrivalCity, setArrivalSuggestions);
    }, 300); // Задержка в 300ms
  ...
  const handleSuggestionClic = (suggestion: string, type: 'departure' | 'arrival') => {
    if (type === 'arrival') {
      setValue('arrival_city', suggestion);
      setArrivalSuggestions([]); // Скрыть предложения
    } else {
      setValue('departure_city', suggestion);
      setDepartureSuggestions([]); // Скрыть предложения
    }
  };
  return (
    <Accordion
