            ...
            {brandOptions && (
              <div className={styles.item_block}>
                ...
                <Select
                  ...
                  disabled={isEditable}
                  onInputChange={(value: SetStateAction<string>) => setBrandSearchInput(value)} // Когда происходит изменение текста
                  onChange={(event) => {
                    const selectedValue = event.target.value; // Извлекаем значение из события
                    setBrandSearchInput(''); // Очищаем текстовый ввод
                    setValue('brand', selectedValue); // Устанавливаем значение в форму
                  }}
                />
