  ...
  const fetchEuropeanCountries = async () => {
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        setTimeout(fetchEuropeanCountries, 5000); // Повторный запрос через 5 секунд
        return; // Прерываем выполнение текущего запроса
      }
      
      // Если статус не 429, пробуем преобразовать данные в JSON
      const data = await response.json();
      console.log(data);
    } catch (error) {
